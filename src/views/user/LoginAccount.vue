      ...
      <a-form-model-item required prop="password">
        <a-input v-model="model.password" size="large" :type="password" autocomplete="false" placeholder="请输入密码">
          ...
          <i
            v-if="delubukejian"
            slot="suffix"
            class="iconfont icon-htmal5icon09"
            @click="changeType()"
            style="color: #969796"
          />
          <i v-else slot="suffix" class="iconfont icon-htmal5icon08" style="color: #969796" @click="changeType()" />
        </a-input>
      </a-form-model-item>
...
export default {
  ...
  methods: {
    changeType() {
      console.log('this.delubukejian', this.delubukejian)
      ...
      this.validateFields(['username', 'password', 'inputCode'], err => {
        if (!err) {
          let loginParams = {
            ...
            password: this.AESEncrypt(this.model.password),
