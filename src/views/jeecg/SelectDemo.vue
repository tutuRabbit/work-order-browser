    ...
    <div class="table-page-search-wrapper">
      <a-form-model ref="form" :model="formData" layout="inline">
            ...
            <a-form-model-item label="高级查询（自定义按钮）">
              <j-super-query :fieldList="superQuery.fieldList">
                ...
                <template v-slot:button="{ isActive, isMobile, open, reset }">
                  <!-- 定义的是普通状态下的按钮 -->
                  <a-button v-if="!isActive" type="primary" ghost icon="clock-circle" @click="open()"
                    >高级查询</a-button
                  >
                  <!-- 定义的当有高级查询条件生效状态下的按钮 -->
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="9"> 输入的值(v-model)：{{ formData.content }} </a-col>
        </a-row>
  ...
  data() {
    return {
      ...
      formData: {
        ...
        jEllipsis:
          '这是一串很长很长的文字段落。这是一串很长很长的文字段落。这是一串很长很长的文字段落。这是一串很长很长的文字段落。',
        jSlider: false,
