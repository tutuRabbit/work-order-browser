  ...
  >
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        <a-col :span="12">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="code" label="出库单号">
            <a-input placeholder="请输入，忽略将自动生成" v-model="model.code" /> </a-form-model-item
        ></a-col>
        <a-col :span="12">
          <a-form-model-item
            label="出库类型"
        ...
        <a-col :span="12">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="s" label="客户名称">
        ...
        <a-col :span="12">
          <a-form-model-item
            ...
            label="供方经办人/送货人"
          >
            <a-input placeholder="供方经办人/送货人" v-model="model.deliveryPerson" />
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="purchaser" label="采购员">
            <a-input placeholder="请输入采购员" v-model="model.purchaser" />
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" prop="supplierUnit" label="供方单位">
            <span>{{ model.supplierUnit }}</span>
          </a-form-model-item>
        </a-col>
        <a-col :span="24">
          <a-form-model-item :labelCol="{ span: 2 }" :wrapperCol="{ span: 21 }" prop="remarks" label="备注">
        ...
        > -->
          <a-tooltip placement="topLeft">
            ...
            <a-checkbox-group v-if="item.type == '复选框'" v-model="item.expands.fieldValue" :placeholder="item.tips">
              <a-row>
                <a-col v-for="(itemx, indexx) in JSON.parse(item.options)" :span="8" :key="indexx">
      ...
      <a-col :span="24">
        <a-form-model-item label="出库明细" :labelCol="{ span: 2 }" :wrapperCol="{ span: 21 }" prop="fields">
          <template>
      ...
      <!-- 子表单区域 -->
      <!-- <a-col>
        <div style="margin-bottom: 10px; padding: 0px; font-size: 16px;color: rgb(24,144,255);">出库明细</div>
        <template>
                ...
                <table style="table-layout: auto;">
                  <thead class="ant-table-thead">
                    <tr>
                      ...
                      <th class="ant-table-cell">出库数量</th>
                  ...
                  <tbody class="ant-table-tbody" v-else>
                    <tr v-for="(item, index) in listCont"  :key="index" >
                      ...
                      <td class="ant-table-cell">
                        <a-input-number v-model="item.libraryNumber" placeholder="请填写"></a-input-number>
                      </td>
                      <td class="ant-table-cell">
  ...
  methods: {
    ...
    handleBase() {
      this.prodSelectionRows.forEach(item => {
        ...
        item.storageLocation = item.storageLocations
    ...
    edit(record) {
      ...
      if (!this.model.id) {
        ...
        this.model.supplierUnit = localStorage.getItem('parentName')
    ...
    loadRecordList() {
      var that = this
      let httpurl = '/sys/api/queryEnableDictItemsByCode?code=out_stock_type'
      let pm = {
        pageNo: 1,
        pageSize: 10000
      }
      ...
      this.listCont.map((item, index) => {
        this.optionsList.map(itemx => {
          if (itemx.code == item.code) {
            ...
            that.listCont[index].type = '2'
            delete that.listCont[index].id
          }
        })
      })
    },
