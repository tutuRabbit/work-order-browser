<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('绩效工资配置')" v-has="'SysRejects:export'"
        >导出
      </a-button>
    ...
    <div>
      ...
      >
        ...
        <template slot="unitPriceS" slot-scope="text, record">
          <a-input-number
            v-if="nickname == '管理员'"
            v-model="record.unitPrice"
            v-on:blur="getData(record.unitPrice, record)"
          ></a-input-number>
    ...
    <sys-merit-pay-modal ref="modalForm" @ok="modalFormOk"></sys-merit-pay-modal>
    <!-- 导入 -->
    <j-modal title="导入绩效工资配置" width="45%" :visible="importVisible" cancelText="关闭" @cancel="handleCancel">
      <div class="table-page-search-wrapper">
        <a-form layout="inline">
          <a-row :gutter="24">
            <a-col :span="24">
              <p>操作步骤:</p>
              <p>1.下载<a @click="downFiles">《导入绩效工资配置模板》</a></p>
            </a-col>
            <a-col :span="24">
    ...
    <!--    导入说明-->
    <import-description-modal :show.sync="importDescriptionVisible" type="meritPay"></import-description-modal>
  </a-card>
</template>
...
import SysMeritPayModal from './modules/SysMeritPayModal'
import { uploadAction, downFile, httpAction } from '@/api/manage'
import ImportDescriptionModal from '@views/work/modules/importDescriptionModal'
import store from '@/store/'

export default {
  name: 'SysMeritPayList',
  ...
  data() {
    ...
    return {
      ...
      url: {
        list: '/longxianda/sysMeritPay/list',
        delete: '/longxianda/sysMeritPay/delete',
        deleteBatch: '/longxianda/sysMeritPay/deleteBatch',
        exportXlsUrl: '/longxianda/sysMeritPay/exportXls',
        importExcelUrl: 'longxianda/sysMeritPay/importExcel', //导入接口
        download: '/attachment/tabAttachment/download?id=7', //绩效工资配置导入模板
  ...
  methods: {
    ...
    downFiles() {
      downFile(this.url.download).then(data => {
        ...
        if (typeof window.navigator.msSaveBlob !== 'undefined') {
          ...
          link.href = url
          link.setAttribute('download', '绩效工资配置导入模板' + Math.round(new Date().getTime()).toString() + '.xlsx')
    ...
    getSuperFieldList() {
      let fieldList = []
      fieldList.push({ type: 'string', value: 'produceCode', text: '产品编码', dictCode: '' })
      fieldList.push({ type: 'string', value: 'produceName', text: '产品名称', dictCode: '' })
      fieldList.push({ type: 'string', value: 'produceSpec', text: '产品规格', dictCode: '' })
      ...
      fieldList.push({ type: 'int', value: 'valuationMothod', text: '计价方法', dictCode: '' })
      fieldList.push({ type: 'int', value: 'standardOutput', text: '标准产出', dictCode: '' })
      fieldList.push({ type: 'int', value: 'standardTaskTime', text: '标准工时(秒)', dictCode: '' })
      fieldList.push({ type: 'double', value: 'unitPrice', text: '工资单价', dictCode: '' })
