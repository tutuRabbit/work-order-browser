...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/troublereport/troubleDocument/list',
        deleteBatch: '/troublereport/troubleDocument/delete',
        download: '/troublereport/troubleDocument/download?troubleId=',
        add: '/troublereport/troubleDocument/add'
      },
      queryParam: {
        documentType: '3'
      },
  ...
  methods: {
    batchDel: function() {
      ...
      if (this.selectedRowKeys.length <= 0) {
        ...
        this.$confirm({
          ...
          onOk: function() {
            that.loading = true
            deleteAction(that.url.deleteBatch, { troubleId: ids })
              .then(res => {
                if (res.success) {
                  //重新计算分页问题
    ...
    handleAdd() {
      this.queryParam.documentType = '3'
