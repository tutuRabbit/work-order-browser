  ...
  <a-spin :spinning="confirmLoading">
    <a-form-model ref="form" :model="tabStock">
      <a-row>
        ...
        <a-col :span="8">
          <a-form-model-item label="日期" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="stockTime">
            <a-date-picker :disabled="formDisabled" v-model="tabStock.stockTime" style="width:100%" />
          </a-form-model-item>
        </a-col>
        <a-col :span="8">
        ...
        <a-col :span="8">
          <a-form-model-item
            ...
            prop="storeHouse"
          >
            <a-select :disabled="formDisabled" v-model="tabStock.storeHouse" placeholder="请选择仓库">
        ...
        <a-col :span="8">
          <a-form-model-item label="供应商" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="supplier">
            <!-- <j-dict-select-tag
              ...
              placeholder="请选择供应商"
            ></j-dict-select-tag> -->
            <a-select :disabled="formDisabled" v-model="tabStock.supplier" placeholder="请选择供应商">
...
export default {
  ...
  methods: {
    changeCheck() {
      // console.log('ddd', this.checkUnit)
      const that = this
      if (this.checkUnit) {
        console.log('添加换算单位，换算金额')
        let pm = {
          title: '换算单位',
          key: 'departId_dictText',
          width: '10%',
          type: FormTypes.normal
        }
        let em = {
        ...
        that.table.columns.splice(6, 0, em)
        that.table.columns.splice(6, 0, pm)
        // that.table.columns = Object.assign({}, this.table.columns)
        console.log('that.set.columns', that.table.columns)
        this.$forceUpdate()
      } else {
        console.log('移除换算单位，换算金额')
        var depart = that.table.columns.findIndex(i => i.title == '换算单位')
        if (depart != -1) {
          that.table.columns.splice(depart, 1)
        }
        var stockManager = that.table.columns.findIndex(i => i.title == '换算数量')
        if (stockManager != -1) {
          that.table.columns.splice(stockManager, 1)
        }
        console.log('that.table.columns', that.table.columns)
        this.$forceUpdate()
      }
      this.$forceUpdate()
    },
    ...
    selectSparePart() {
      // if (this.tabStock.storeHouse) {
      // this.queryParam.storeHouse = this.tabStock.storeHouse
      this.searchQuery()
      this.spareVis = true
      // } else {
    ...
    add() {
      ...
      this.tabStock = Object.assign({}, this.modelDefault)
      this.visible = true
    },
    edit(record) {
      getAction('/info/tabStock/queryById?id=' + record.id).then(res => {
        if (res.success) {
          ...
          this.tabStock = res.result.stock || []
        }
      })
      this.visible = true
