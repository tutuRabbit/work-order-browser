<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-dropdown>
        <a-menu slot="overlay">
          <a-menu-item key="1" @click="handleExportXls('库存信息_入库主表')">导出主表</a-menu-item>
          <a-menu-item key="1" @click="handleExportDetailXls('库存信息_入库明细表')">导出明细</a-menu-item>
        </a-menu>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          align: 'center',
          dataIndex: 'stockType'
          // customRender: inStockType => (inStockType ? filterMultiDictText(this.dictOptions['stockType'], inStockType) : '')
        },
        {
          title: '部门',
          align: 'center',
  ...
  created() {
    ...
    getAction('/stock/tabStore/list').then(res => {
      if (res.success) {
        this.$set(this.dictOptions, 'stockType', res.result.records)
        ...
        on: {
          click: () => {
            getAction('/info/tabStock/queryById?id=' + record.id).then(res => {
              console.log('请选中一条数据！.success', res)
              if (res.success) {
