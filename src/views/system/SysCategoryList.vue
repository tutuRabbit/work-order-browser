<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button @click="handleAdd" type="primary" icon="plus" v-has="'category:add'">新增</a-button>
      <!-- <a-button type="primary" icon="download" @click="handleExportXls('分类字典')">导出</a-button>
      <a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader"  :action="importExcelUrl" @change="handleImportExcel">
        <a-button type="primary" icon="import">导入</a-button>
      </a-upload> -->
      <a-dropdown v-if="selectedRowKeys.length > 0" v-has="'category:delete'">
        <a-menu slot="overlay">
    ...
    <div>
      ...
      >
        <span slot="action" slot-scope="text, record">
          <a @click="handleEdit(record)" v-has="'category:edit'">编辑</a>
          <a-divider type="vertical" v-has="'category:edit'" />
          <a-popconfirm title="确定删除吗?" @confirm="() => handleDelete(record)" v-has="'category:delete'">
            <a>删除</a>
          </a-popconfirm>
          <a-divider type="vertical" v-has="'category:delete'" />
          <a @click="handleAddSub(record)" v-has="'category:addSub'">添加下级</a>
        </span>
      </a-table>
    </div>
